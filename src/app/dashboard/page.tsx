...
export default function Dashboard() {
  ...
  const getPlanName = (planId: number) => {
    const planNames = ['Unknown', 'Basic', 'Standard', 'Premium', 'Ultimate'];
    return planNames[planId] || 'Unknown';
  };

  // Subscription is only renewable once it's within this many days of expiring (or already ended)
  const RENEWAL_WINDOW_DAYS = 7;
  const isRenewalEligible = () => getDaysRemaining() <= RENEWAL_WINDOW_DAYS;

  const handleRenewClick = () => {
    if (!isRenewalEligible()) {
      alert(
        `Your subscription still has ${getDaysRemaining()} day(s) left. You can renew once you're within ${RENEWAL_WINDOW_DAYS} days of your renewal date.`
      );
      return;
    }
    router.push('/subscription');
                ...
                <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 sm:p-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center py-3 border-b border-white/10">
                      ...
                      <span className="text-white font-medium">{getPlanName(subscription.planId)}</span>
                    </div>
                ...
                <button
                  ...
                  title={
                    isRenewalEligible()
                      ...
                      : `Available ${RENEWAL_WINDOW_DAYS} days before your subscription ends`
                  }
                  className={`group relative inline-flex items-center justify-center px-8 py-4 text-lg font-semibold text-white rounded-2xl shadow-2xl transform transition-all duration-300 w-full sm:w-auto ${
                    isRenewalEligible()
                      ? 'bg-gradient-to-r from-blue-600 to-purple-600 hover:shadow-blue-500/25 hover:scale-105'
                      : 'bg-gray-600/40 border border-white/10 opacity-60 cursor-not-allowed hover:scale-100'
                  }`}
                >
                  {isRenewalEligible() && (
